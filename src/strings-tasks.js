...
function isPalindrome(str) {
  const search = str.toLowerCase().replace(/[^a-zA-Z]/g, '');
  return search === search.split('').reverse().join('');
}
...
function invertCase(str) {
  return str
    .split('')
    .map((x) => (x.charCodeAt(0) > 91 ? x.toUpperCase() : x.toLowerCase()))
    .join('');
}
